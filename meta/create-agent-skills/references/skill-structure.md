...
| `user-invokable` | `true` for user-only invocation (no auto-loading). Use for side-effect workflows. |
| `user-invokable` | `false` to hide from `/` menu. Use for background knowledge. |
...
- [ ] `user-invokable: true` if skill has side effects (never `disable-model-invocation`)
