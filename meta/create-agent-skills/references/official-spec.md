...
| `user-invokable` | No | Set `true` to make user-invokable only (Claude won't auto-load). Use for manual workflows. Default: skills are both user- and model-invokable. |
...
| `user-invokable: true` | Yes | No | Description in context, full skill loads when user invokes via `/name` |
| `user-invokable: false` | No | Yes | Description always in context, full skill loads when invoked |
