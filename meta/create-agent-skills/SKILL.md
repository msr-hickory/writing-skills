...
| `user-invokable` | No | Set `true` to make user-invokable only (Claude won't auto-load). Use for manual workflows like `/deploy`, `/commit`. Default: skills are both user- and model-invokable. |
| `user-invokable` | No | Set `false` to hide from `/` menu. Use for background knowledge. Default: `true`. |
...
| `user-invokable: true` | Yes | No | Description in context, loads only when user invokes via `/name` |
| `user-invokable: false` | No | Yes | Description always in context, loads when relevant |

**Use `user-invokable: true`** for workflows with side effects: `/deploy`, `/commit`, `/triage-prs`, `/send-slack-message`. You don't want Claude deciding to deploy because your code looks ready.

**Use `user-invokable: false`** for background knowledge that isn't a meaningful user action: coding conventions, domain context, legacy system docs.

> **Note:** `disable-model-invocation: true` is deprecated — it blocks the Skill tool entirely, preventing even `/name` invocation. Use `user-invokable: true` instead.
...
- **Manual workflow** → command with `user-invokable: true`
- **Background knowledge** → skill without invocation flags (or `user-invokable: false`)
...
- [ ] `user-invokable: true` if it has side effects (never `disable-model-invocation`)
...
- **Missing invocation control** - Side-effect workflows need `user-invokable: true`
